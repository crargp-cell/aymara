...
import { CheckCircle2, Lock, FileQuestion, BookOpen, Sparkles, Eye } from "lucide-react";
...
export default async function MapPage() {
  ...
  if (esDocente) {
    // El docente revisa el mapa del paralelo que eligió: sin uno seleccionado no
  ...
  return (
    <div className="space-y-6">
      <Topbar
        ...
        subtitle={esDocente ? `${paralelo.nombre} · ${total} nodos` : `${paralelo.nombre} · ${done}/${total} completados`}
      />

      {esDocente ? (
        <>
          <ParaleloSwitcher actual={paralelo} opciones={opciones} back="/map" />
          <div className="glass rounded-xl px-4 py-3 flex items-center gap-2 text-sm border border-amber-400/25 bg-amber-400/5">
            <Eye className="h-4 w-4 text-amber-300 shrink-0" />
            <span className="text-muted-foreground">
              Vista de revisión: ves el recorrido tal como lo armaste, con todo desbloqueado. Tu avance no se registra.
            </span>
          </div>
        </>
      ) : (
      ...
      {total === 0 ? (
        <div className="glass rounded-2xl p-8 text-center text-sm text-muted-foreground">
          Este paralelo todavía no tiene lecciones ni exámenes en el mapa.
        </div>
      ) : (
        <div className="relative glass rounded-2xl p-6 overflow-hidden" style={{ height }}>
          ...
          {nodes.map((n, i) => {
            ...
            const content = (
              <>
                <div className={`relative h-20 w-20 rounded-full flex items-center justify-center text-lg font-bold border-4 transition-all glass ${cls} ${n.unlocked && !n.done && !esDocente ? "animate-float" : ""}`}>
                  ...
                  {n.unlocked && !n.done && !esDocente && <span className="absolute inset-0 rounded-full border-2 border-primary/50 animate-ping" />}
                </div>
                <div className={`glass rounded-lg px-2 py-1 text-xs text-center max-w-[150px] leading-tight ${!n.unlocked ? "opacity-60" : ""}`}>
                  ...
                  {n.nota && <p className="text-[10px] text-amber-300/80 mt-0.5 line-clamp-2">{n.nota}</p>}
                </div>
