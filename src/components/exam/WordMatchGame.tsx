...
export function WordMatchGame({ examId, timeLimitSeconds, words }: { examId: number; timeLimitSeconds: number; words: Word[] }) {
  ...
  return (
    <div className="space-y-4 pb-28 md:pb-0">
            ...
            {pool.map((w) => {
              ...
              return (
                <button
                  ...
                  disabled={finished}
                  className={`rounded-xl p-3 text-center font-semibold text-sm transition-all panel hover:-translate-y-0.5 cursor-grab active:cursor-grabbing ${isSelected ? "ring-2 ring-primary scale-[1.02]" : ""} ${isShaking ? "animate-[shake_0.4s_ease-in-out]" : ""}`}
                >
                  {w.text}
        ...
        <aside className="hidden md:flex md:flex-col gap-3">
          {basketList.map(({ cat, total, placedInCat }) => (
            <div
              ...
              className={`panel rounded-xl p-3 transition-all cursor-pointer ${dragOverCat === cat ? "ring-2 ring-primary scale-[1.02]" : ""} ${selected.length > 0 ? "hover:ring-2 hover:ring-primary/50" : ""}`}
            >
      ...
      <div className="md:hidden fixed bottom-0 inset-x-0 z-40 bg-background/95 backdrop-blur border-t border-border p-2">
        <div className="flex gap-2 overflow-x-auto pb-1">
          {basketList.map(({ cat, total, placedInCat }) => (
            <div
              ...
              onDrop={(e) => handleBasketDrop(e, cat)}
              onClick={() => handleBasketClick(cat)}
              className={`shrink-0 w-24 panel rounded-xl p-2 text-center transition-all ${dragOverCat === cat ? "ring-2 ring-primary scale-105" : ""}`}
            >
      ...
      {finished && result && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-foreground/50 backdrop-blur-sm p-4">
          <div className="panel-strong rounded-2xl p-8 max-w-sm w-full text-center space-y-3">
            ...
            {result.arCardId && <p className="text-sm font-medium">🎯 ¡Desbloqueaste una tarjeta AR!</p>}
            <p className={`text-sm ${result.passed ? "text-[var(--ruta-completado)]" : "text-[var(--ruta-futuro)]"}`}>{result.passed ? "Aprobaste el examen." : "No alcanzaste el puntaje mínimo."}</p>
